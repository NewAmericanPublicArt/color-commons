...
// -- GLOBALS (dataobjs)
	...
	node = root,
        colorize = function(term) {
	    var color = d3.color("#FFFFFF");
	    var lookup = COLORS[term.toLowerCase()];
	    if (lookup != null) {
	        color = d3.color(lookup);
	    }
	    return color;
        },
	arc = d3.arc()
...
within loadburst()
    ...
    function arcTweenData(a, i) { // When switching data: interpolate the arcs in data space.
		...
		if (i == 0) { // If we are on the first arc, adjust x domain to match root node @ current zoom level (Only 1x)
...
// LOADER for ABOUT - provides last-updated information
